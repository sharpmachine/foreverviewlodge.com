...
/*
if( !fj_check_video_if_exist( get_option("oqey_default_video") )){    
    global $wpdb;
    $oqey_video = $wpdb->prefix . "oqey_video";		
    $r = $wpdb->get_row("SELECT id FROM $oqey_video WHERE status!=2 LIMIT 0,1");
    if(!empty($r->id)){
        update_option("oqey_default_video", $r->id);
    }else{
        update_option("oqey_default_video", "1");        
    }
}  
*/
...
if(isset($_REQUEST['extvideoname']) && isset($_REQUEST['savevideo'])){
   ...
   if($sql){
     ...
     $mesaj = '<div class="wrap">
               ...
               <p>External video link has been registered.</p>
               </div>
               </div>';
     }else{
...
if(isset($_REQUEST['delete']) && isset($_REQUEST['ID']) ){
    ...
    $delete_video = $wpdb->query( $wpdb->prepare( "DELETE FROM $oqey_video WHERE id = %d ", esc_sql($_REQUEST['ID']) ));
    
    $video_file = OQEY_ABSPATH."wp-content/oqey_gallery/video/".oqey_getBlogFolder($wpdb->blogid).trim($_REQUEST['title']);
    $path = pathinfo($_REQUEST['title']);    
    $img_file = $path['filename'].".jpg";
    $video_file_img = OQEY_ABSPATH."wp-content/oqey_gallery/video/".oqey_getBlogFolder($wpdb->blogid).$img_file;

    if(file_exists($video_file)){
    
       $delete = unlink($video_file);

       if(file_exists($video_file_img)){
          
          $delete_img = unlink($video_file_img);
       }
    }

  if($delete){ 
    $mesaj = '<div class="wrap"><div class="updated fade" id="message" style="width:881px;"><p>Deleted</p></div></div>'; 
   }else{ 
    $mesaj = '<div class="wrap"><div class="error fade" id="message" style="width:881px;"><p>Error, please try again.</p></div></div>'; 
    }
}
*/
...
<div class="postbox" style="width:900px;">   
    ...
    <tr>
    <td width="128" height="50" align="center">Upload video</td>
...
?>
  ...
  ?> 
    ...
    <form id="scaner" name="scaner" method="post" action="<?php echo admin_url('admin.php?page=oQeyVideo&scaner=true'); ?>">     
        <input name="scanfolder" id="scanfolder" value="Magic Scan" type="submit">              
    </form>
    ...
    <td width="65" valign="middle"> 
        <a href="#add_video" class="videolink" id="videolink">
          <img src="<?php echo oQeyPluginUrl().'/images/exlinkvideo.png'; ?>" title="add an external video file" />
        </a>
    </td>
    </tr>
    </table>
...
<table width="880" border="0" cellspacing="0" cellpadding="0" style="margin:10px;">
  <tr>
    <td height="26" colspan="3">&nbsp;Add an external video link (http://domain.com/my-video-file.flv)</td>
    </tr>
  <tr>
    <td width="614"><input name="extvideoname" type="text" id="extvideoname" style="width:600px;"/></td>
    <td width="266" colspan="2"><input type="submit" name="savevideo" id="savevideo" value="Add video" /></td>
    </tr>
</table>
</form>
...
<table width="900" border="0" cellspacing="0" cellpadding="0" id="musictable" class="tablesorter">
  <tbody id="sortable">
    ...
    if(!empty($img_file) && is_file($imgroot)){ 
        ...
        echo '<img src="'.oQeyPluginUrl().'/images/with-photo.png" height="100" width="150" title="Edit photo" /></div>';
    ...
    /*
    if(get_option('oqey_default_video')==$video->id){
        
        $defaultimg = '<img src="'.oQeyPluginUrl().'/images/checked.png" title="Default video" height="23" width="23" class="defaultvideo" id="v'.$video->id.'"/>';
     
     }else{
        
        $defaultimg = '<img src="'.oQeyPluginUrl().'/images/unchecked.png" title="Set as default video" height="23" width="23" class="setdefaultvideo" id="v'.$video->id.'"/>';
    ...
    <td width="635" style="padding-top:10px; vertical-align:top; text-align:left; line-height:1.5;" >
        ...
        <tr>
         <td width="90" style="height: 17px; vertical-align: middle;">Title:</td>
         <td width="502"><span class="dblclick" id="video_title_<?php echo $video->id; ?>"><?php echo $video->title; ?></span></td>
        </tr>
        <tr>
         <td style="height: 17px; vertical-align: middle;">Shortcode:</td>
    ...
    <td width="32" align="center" valign="middle" class="lasttd"> 
      ...
      <img src="<?php echo oQeyPluginUrl(); ?>/images/remove_button.png" width="22" height="22" title="Move to trash this video"/></a>    
    </td>
  </tr>
  <?php $i++; //hiddenm
  }
...
Notes: <br />
         ...
         * You may upload new video files directly to your plugin video directory via ftp.<br />
         * Your video folder location:<b> <?php echo get_option('siteurl').'/wp-content/oqey_gallery/video/'.oqey_getBlogFolder($wpdb->blogid); ?></b>    </div>
</div> 
...
function VideoDelete(id, title){
        ...
        jQuery.post( ajaxurl, { action :"oQeyTrashVideoFile", id: id },
            function(data){                
                ...
                jQuery("#save").html('<div class="updated fade" id="message"><p>' + data.info + ' <a href="#undo" id="undovideo">undo<\/a><\/p><\/div>');
...
<input type="button" class="button-primary" style="width:50px; margin-top:5px;" value="<?php _e('Save') ?>" onclick="SaveVideoImage(); return false;" />
</div>
</div>
...
Use this section to add videos to your website. We strongly recommend to first convert your videos to *.mp4 video using this free utility - Miro Video Converter <a href="http://www.mirovideoconverter.com/" target="_blank">http://www.mirovideoconverter.com</a> Converting your video content to mp4 will make it accessible from both flash enabled browsers and html5 devices. It is also recommended to upload video content via ftp when uploading files of a size above your server maximum upload limit.
</div>
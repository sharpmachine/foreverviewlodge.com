...
?>
<div class="wrap">
        <h2>Trash <?php if(!empty($get_music) || !empty($get_skins) || !empty($get_g_list) || !empty($galls) || !empty($videos) ){ echo "| <a href='#empty_trash' id='empty_trash' class='empty_trash'>Empty Trash</a>"; } ?></h2>
...
<div id="tabs" style="display:none;">
	<ul id="taburile">
        ...
        <?php if(!empty($videos)){ ?><li><a href="#tabs-5">Video</a></li><?php } ?>
	</ul>
...
?>
          <tr id="video_tr_<?php echo $video->id; ?>">
			 ...
			 <p><a href="#restore" class="restorevideo" onclick="restoreVideo('<?php echo $video->id; ?>'); return false;">Restore</a> | 
             <a href="#delete_this_video" class="delete_this_video" onclick="deleteVideo('<?php echo $video->id; ?>', '<?php echo $video->video_link; ?>'); return false;">Delete Permanently</a></p>
			 </td>
           </tr>
<?php } 
...
function deleteVideo(id, name){ 
   ...
   function(data){
			...
			jQuery("#save").hide().html('<p class="updated fade">Video was deleted.<\/p>').fadeIn("slow"); 
   });
...
jQuery("#empty_trash").click(function(){
	    if(confirm("Are you sure about erasing these data?")){ 

...
class DT_latest_photo_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		global $wpdb, $dt_where_filter_param;
		$dt_where_filter_param = sprintf( 'SELECT ID FROM %s WHERE post_type="%s" AND post_status="publish"', $wpdb->posts, 'dt_gallery_plus' );
...
//				$img_file = str_replace(site_url(), '', $img[0]);
				$tmp_src = explode($_SERVER['SERVER_NAME'], $img[0]);
				$tmp_src = isset($tmp_src[1])?$tmp_src[1]:$img[0];
				$photo_t_src = get_template_directory_uri()."/thumb.php?src={$tmp_src}&w=47&h=47&zc=1";
				
				$photo_b_src = esc_url($img[0]);
				printf( '<a href="%s" class="%s" title="%s"><img width="%d" height="%d" src="%s"/><i></i></a>',
					esc_url($photo_b_src),
					'alignleft prettyPhoto',
					esc_attr(apply_filters('the_title', $photo->post_excerpt)),
					47, 47,

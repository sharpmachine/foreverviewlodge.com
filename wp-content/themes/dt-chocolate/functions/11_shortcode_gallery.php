...
function dt_gallery_custom($attrs) {
   ...
   global $post, $wp_query;
	...
	$cover = array_shift($attachments);
	
	global $attachments_counter;
	$attachments_counter++;
	
   $pic = wp_get_attachment_image_src( $cover->ID , 'large' );
...
//   $pic = str_replace(site_url(), "", $pic);
   $tmp_src = explode($_SERVER['SERVER_NAME'], $pic);
   $tmp_src = isset($tmp_src[1])?$tmp_src[1]:$pic;
   $pic = get_template_directory_uri().'/thumb.php?src='.$tmp_src.'&amp;w='.$w.'&amp;h='.$h.'&amp;zc=1';

	if ( !empty($attrs["title"]) ) {
		$new_title = $attrs["title"];
	} else {
		$new_title = __('View all pictures');
		$attrs["title"] = '';
	}
...
<div class="big_gallery">
  ...
  <a href="#" class="go_back">'.__('Back').'</a>
...
//       $orig_image = str_replace(site_url(), "", $orig_src);
	   $tmp_src = explode($_SERVER['SERVER_NAME'], $orig_src);
	   $tmp_src = isset($tmp_src[1])?$tmp_src[1]:$orig_src;

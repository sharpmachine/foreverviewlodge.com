...
function woo_load_tumblog_libraries() {
   $pluginurl = get_template_directory_uri()."/plugins/woo-tumblog/";
	wp_enqueue_script('newscript', $pluginurl .'functions/js/tumblog-ajax.js', array('jquery', 'jquery-form'));
	wp_enqueue_script('nicedit', $pluginurl .'functions/js/nicEdit.js');
 	wp_enqueue_script('phpjs', $pluginurl .'functions/js/php.js');
 	wp_enqueue_script('datepicker', $pluginurl .'functions/js/ui.datepicker.js',array('jquery'));
}    
...
function woo_load_tumblog_css($hook) {
   $pluginurl = get_template_directory_uri()."/plugins/woo-tumblog/";
	if ($hook == 'post.php' OR $hook == 'post-new.php' OR $hook == 'page-new.php' OR $hook == 'page.php' OR (isset($_REQUEST['page']) && $_REQUEST['page'] == 'woothemes_sbm')) {
		// Do Nothing
	} else {
		echo "<link rel='stylesheet' id='colors-tabs' href='".$pluginurl."functions/css/tumblog_admin_styles.css' type='text/css' media='all' />";
		echo "<link rel='stylesheet' id='colors-tabs' href='".$pluginurl."functions/css/jquery-ui-datepicker.css' type='text/css' media='all' />";
	}   
}
	...
	switch ($type) 
	{
    	case 'note':
			...
			//Insert the note into the database
  			...
  			// set size
  			$size = $data['tumblog-size'];
  			global $post;
  			$post = get_post($post_id);
         global $postoptions;
         $postoptions->post_id = $post_id;
         $postoptions->set_post_option('post_size', $size);
  			// end set size
...
function woo_tumblog_dashboard_widget_output() {
	...
	if (current_user_can('publish_posts')) {
		...
		?>
		<script type="text/javascript" src="<?php echo get_template_directory_uri().'/plugins' . '/woo-tumblog'; ?>/functions/js/ajaxupload.js"></script>
		...
		jQuery(document).ready(function(){
			...
			jQuery('.date-picker').each(function (){
				jQuery('#' + jQuery(this).attr('id')).datepicker({showOn: 'button', buttonImage: '<?php echo get_template_directory_uri().'/plugins' . '/woo-tumblog';?>/functions/images/calendar.gif', buttonImageOnly: true});
			});
			...
			jQuery('#articles-menu-button').click(function ()
			{
				...
				jQuery('#sizes-fields').removeAttr('class');
				...
				if (nicEditors.findEditor('test-content') == undefined) {
					myNicEditor = new nicEditor({ buttonList : ['bold','italic','underline','ol','ul','left','center','right','justify','link','unlink','strikeThrough','xhtml','image'], iconsPath : '<?php echo get_template_directory_uri().'/plugins' . '/woo-tumblog'; ?>/functions/images/nicEditorIcons.gif'}).panelInstance('test-content');
				} else {
	...
	<div id="tumblog-post">
		...
		<img id="ajax-loader" src="<?php echo get_template_directory_uri().'/plugins' . '/woo-tumblog'; ?>/functions/images/ajax-loader.gif" />
		...
		<div id="sizes-fields" class="hide-fields" style="clear:both;padding-top:10px;">
			<h4 id="tumblog-sizes-title"><label>Size</label></h4>
			<div>
			   <label>
   				<input name="tumblog-size" value="s" type="radio" /> S
				</label>
			   <label>
   				<input name="tumblog-size" value="m" type="radio" checked="checked" /> M
				</label>
			   <label>
   				<input name="tumblog-size" value="l" type="radio" /> L
				</label>
			</div>
		</div>

...
function woo_tumblog_image_content($post_id) {
   ...
   //get_the_title();
	...
	if (false) {
  		$content_tumblog = '<a href="'.get_post_meta($post_id, "image", true).'" class="prettyPhoto" title="'.htmlspecialchars(get_the_title()).'">'.woo_tumblog_image('key=image&width='.$w.''.$height.'&link=img&return=true&class=alignleft').'</a>';  
  	} else { 
      ...
      if ($size[0] > 0)
      {
         ...
         if ($is_large)
         {
            ...
            if (!is_single())
          	...
          	$content_tumblog .= '
          	<a href="'.$im.'" class="prettyPhoto" title="'.htmlspecialchars(get_the_title()).'">'.'<img src="'.$thumb.'" width="'.$w.'" height="'.$h.'" alt="" class="hd" />'.'</a>
         ...
         {
          	$content_tumblog = '
          	<a href="'.$im.'" class="prettyPhoto" title="'.htmlspecialchars(get_the_title()).'">'.'<img src="'.$thumb.'" width="'.$w.'" height="'.$h.'" alt="" class="alignleft" />'.'</a> 
          	';
         }
      }
      else
      {
       	$content_tumblog = '
       	<a href="'.$im.'" class="prettyPhoto" title="'.htmlspecialchars(get_the_title()).'">'.woo_tumblog_image('key=image&width='.$w.'&height=250&link=img&return=true&class=hd').'</a>
         <div class="hr_w bot"><i></i></div> 
       	';
      }
	}

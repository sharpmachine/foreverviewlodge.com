...
class WooTumblog {
	...
	function WooTumblog() {
		...
		update_option('woo_tumblog_version', '2.0.2');
	...
	function woo_tumblog_header_code() {
		...
		//Audio Player
		echo '<script type="text/javascript" src="'.$pluginurl.'functions/swfobject.js"></script>';
		//Frontend styles
		echo '<link rel="stylesheet" type="text/css" media="all" href="'.$pluginurl.'functions/css/tumblog_frontend_styles.css" />';
	...
	function woo_tumblog_options() {
		...
		$pluginpath = dirname( __FILE__ );
		//Audio Player
		//$plugin_url = WP_PLUGIN_URL. '/' . plugin_basename( $pluginpath );
		...
		if ($resize_thumb == '') {
			update_option('woo_resize', 'false');
		}
		//The Options Form	
		?>
		...
		<div id="woo_container" class="wrap">
			...
			<div id="header">
            	...
            	<div class="logo">
                	<img alt="WooThemes" src="<?php echo $plugin_url; ?>/../functions/images/logo.png"/>
                </div><!-- /.logo -->
                
                <div class="theme-info">
                    <span class="theme"><?php echo('WooTumblog Plugin Options'); ?></span>
                    <span class="framework"><?php echo('Version'); ?> <?php echo get_option('woo_tumblog_version'); ?></span>
                </div><!-- /theme-info -->
            ...
            <div id="support-links">
                ...
                <ul>
                    <li class="changelog"><a title="Plugin Changelog" href="<?php echo $plugin_url; ?>/../changelog.txt"><?php echo('View Changelog'); ?></a></li>
                    <li class="docs"><a title="Plugin Documentation" href="http://www.woothemes.com/support/wootumblog/"><?php echo('View Plugin Docs'); ?></a></li>
                    <li class="forum"><a href="http://wordpress.org/support/" target="_blank"><?php echo('Visit WordPress Forum'); ?></a></li>
			...
			<div id="main">
				...
				if (isset($_REQUEST['submit'])) {
					...
					if ($_REQUEST['woo_tumblog_content_method']) {
						update_option('woo_tumblog_content_method', $_REQUEST['woo_tumblog_content_method']);
						$ok = true;		
					}
					...
					if ($_REQUEST['woo_resize']) {
						update_option('woo_resize',$_REQUEST['woo_resize']);
						$ok = true;
					} else {
						update_option('woo_resize','false');
						$ok = true;
					...
					if ($ok) {
						...
						<div id="message" class="updated fade">
							<p><?php echo('Options saved'); ?>.</p>
						</div>
						<?php 
					} else {
						?>
						<div id="message" class="error fade">
							<p><?php echo('Failed to save options'); ?>.</p>
						</div>
						<?php 	
					}
				} 
				...
				<form method="post">
					...
					<?php if ( function_exists('set_post_format') ) { ?>
					
					<p>
				    	<label for="woo_tumblog_content_method"><?php echo('Tumblog Content Method'); ?>: </label>
				    	<select id="woo_tumblog_content_method" name="woo_tumblog_content_method">
				    		<?php 
				    		foreach ($content_option_array as $key => $value) {
				    			if ($default_content_option == $key) {
					...
					<p>
				    	<label for="woo_custom_rss"><?php echo('Use Custom Tumblog RSS Feed'); ?>: </label>
				    ...
				    <p>
				    	<label for="woo_image_link_to"><?php echo('Tumblog Image Posts Link to'); ?>: </label>
				    ...
				    <p>
				    	<label for="woo_tumblog_width">
				    		<?php echo('Tumblog Image Width'); ?>:
				    	</label>
				    	<input class="" type="text" name="woo_tumblog_width" value="<?php echo $default_image_width_option; ?>" /><span class="px">px</span>
				    	<span class="secondary-label"><?php echo('Height is dynamically calculated'); ?></span>
				    </p>
				    
				    <p>
				    	<label for="woo_resize">
				    		<?php echo('Enable Dynamic Image Resizer'); ?>:
				    	</label>
				    	<input type="checkbox" name="woo_resize" id="woo_resize" value="true" class="" <?php if ($resize_thumb == 'true') { ?>checked="checked"<?php } ?>><span class="secondary-label"><?php echo('This will enable the thumb.php script which dynamically resizes images on your site.'); ?></span>
				    </p>
	        ...
	        foreach ($woo_metaboxes as $woo_metabox) { // On Save.. this gets looped in the header response and saves the values submitted
	            ...
	            elseif($woo_metabox['type'] == 'upload') // So, the upload inputs will do this rather
	                ...
	                $override['action'] = 'editpost';
	                    ...
	                    elseif(isset($_POST[ $id ]) && $_POST[ $id ] == '')  { delete_post_meta($pID, $id, get_post_meta($pID, $id, true));
	                    }
	                }
	    ...
	    if ( function_exists('add_meta_box') ) {
	    	...
	    	if ( function_exists('get_post_types') ) {
				...
				foreach ($custom_post_list as $type){
				   if ($type == 'dt_gallery') continue;
				   if ($type == 'dt_gallery_plus') continue;
				   if ($type == 'dt_portfolio') continue;
				   if ($type == 'page') continue;
				   if ($type == 'main_slider') continue;
